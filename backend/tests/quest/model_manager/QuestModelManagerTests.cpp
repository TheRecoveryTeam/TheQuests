...
ASSERT_TRUE(inserted_quest
.find("error") == inserted_quest.
...
ASSERT_TRUE(data
== received_quest) << "The quest_manager has't been correctly added to the database";
}
...
nlohmann::json received_quest = nlohmann::json::parse(quest_manager->get(query.dump()));
ASSERT_EQ((*id_list)[0], received_quest["_id"]["$oid"]
) << "Wrong quest_manager received";
}
