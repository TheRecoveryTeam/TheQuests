...
ASSERT_TRUE(inserted_card
.find("error") == inserted_card.

end()

) << "Get error on correct data";
nlohmann::json query = {
...
) << "The card_manager has't been added to the database";
}
...
) << "Doesn't return error on not existing quest id";
}
...
nlohmann::json received_card = nlohmann::json::parse(card_manager->get(query.dump()));
ASSERT_EQ((*card_id_list)[0], received_card["_id"]["$oid"]
) << "Wrong card received";
}
...
) << "Doesn't return error on incorrect caed id";
}
...
== result) << "Incorrect get end next card";
}
...
ASSERT_TRUE(expected_result
== result) << "Incorrect get next card";
}
...
ASSERT_TRUE(expected_result
== result) << "Incorrect get lose next card";
}
