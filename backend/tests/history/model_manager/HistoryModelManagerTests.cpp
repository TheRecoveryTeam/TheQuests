...
ASSERT_EQ((*history_id_list)[0], received_history["id"]
) << "Wrong history received";
}
...
nlohmann::json received_history = nlohmann::json::parse(history_manager->get(query.dump()));
ASSERT_TRUE(received_history
.find("error") != received_history.
...
ASSERT_TRUE(received_quest
.find("error") != received_quest.
...
ASSERT_TRUE(received_new_history
.find("error") == received_new_history.
...
ASSERT_TRUE(
        history_update_query["cardId"]
== history["cardId"] && history_update_query["resources"] == history["resources"])
